...
int dfs(pi v, pi parent, vector<vector<bool>> vis, const vector<vector<vector<pi>>> &G,const vector<vector<int>> &map) {
        auto [_i, _j] = v;
        vis[_i][_j] = true;
        if(map[_i][_j] == 9) return 1;

        int ret = 0;
        for(auto [i, j] : G[_i][_j]){
            pi tmp = {i, j};
            if(tmp != parent && !vis[i][j])
                ret += dfs(tmp, v, vis, G, map);
        }
