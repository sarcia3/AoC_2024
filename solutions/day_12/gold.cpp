            ...
            for(int x=0; x<n; x++){
                for(int y=0; y<m; y++){
                    ...
                    if(!is_legal({x-1, y}) || num != vis[x-1][y]){
                        sides[{x-1, y, 0}] = true;
                        if(!(sides[{x-1, y-1, 0}] || sides[{x-1, y+1, 0}])) tmp ++;
                    }
                    if(!is_legal({x+1, y}) || num != vis[x+1][y]){
                        sides[{x+1, y, 1}] = true;
                        if(!(sides[{x+1, y-1, 1}] || sides[{x+1, y+1, 1}])) tmp ++;
                    }
                    if(!is_legal({x, y-1}) || num != vis[x][y-1]){
                        sides[{x, y-1, 2}] = true;
                        if(!(sides[{x-1, y-1, 2}] || sides[{x+1, y-1, 2}])) tmp ++;
                    }
                    if(!is_legal({x, y+1}) || num != vis[x][y+1]){
                        sides[{x, y+1, 3}] = true;
                        if(!(sides[{x-1, y+1, 3}] || sides[{x+1, y-1, 3}])) tmp ++;
                    }
                }
            }
